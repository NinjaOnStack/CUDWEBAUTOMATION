export class shipmentRouting {
    ...
    weblocators = {
        ...
        searchInputByReferenceNumber: 'input[placeholder="Search by Shipment.referenceNumber"]',
        ...
        cy.get('@forwarderReferenceNumber').then(fwdRefNo => {
            cy.get(this.weblocators.firstRoutingVehicleCard).first().then($source => {
                cy.get('div.route-shipments').first().then($target => {
                    ...
                    cy.wait(10000);
                    cy.get(this.weblocators.defaultButton).contains(' Complete Route ').click({ force: true });
                    cy.get(this.weblocators.confirmRouteCompletionButton).contains(' Confirm ').click();
                    cy.wait(8000);
                    cy.get(this.weblocators.searchInputByReferenceNumber).scrollIntoView().should('be.visible').type(fwdRefNo);
                    cy.get('tr.shipment-list-item').contains('td', fwdRefNo);
                });
            });
        });
    }
